...
<?php
include 'db.php';
$query=mysqli_query($conn,"SELECT * FROM qdb WHERE status='1' AND no<'".$_SESSION['qid']."' ORDER BY no DESC LIMIT 3;");
$yes_we_have=0;
$jsarray = array();
...
$_SESSION['qid']=$last_qid;
if($yes_we_have)
echo "<button type='button' onClick='showmore()' class='btn btn-lg btn-primary'>Show more</button>";
else
{echo '<div class="list-group"><a href="javascript:void(0)"  class="list-group-item active"><i class="fa fa-list-ul animated"></i>&nbsp;Wow! looks like you found this really interesting! Nice, but you have read all the Questions. Keep Visting Pollsview for more fun.</a></div>';
echo "<button type='button' onclick='showhome()' class='btn btn-lg btn-primary'>Take me back to Home</button>";
}
